...
BEGIN
  ...
  INSERT INTO portal.personsection (por_id, pse_name, pse_order)
    VALUES (prm_por_id, prm_name, new_order)
    RETURNING pse_id INTO ret;
  RETURN ret;
END;
$$;
COMMENT ON FUNCTION personsection_add(prm_token integer, prm_por_id integer, prm_name text) IS 
'Add a section to an dossier view of a portal';

CREATE OR REPLACE FUNCTION personsection_list(prm_token integer, prm_por_id integer)
...
END;
$$;
COMMENT ON FUNCTION personsection_list(prm_token integer, prm_por_id integer) 
IS 'List the sections of a dossier view';
...
IS 'Rename a section of a dossier view';
...
IS 'Delete a section of a dossier view';
...
BEGIN
  PERFORM login._token_assert(prm_token, '{structure}');
  SELECT por_id INTO por FROM portal.personsection WHERE pse_id = prm_id;
  IF NOT FOUND THEN
    RAISE EXCEPTION USING ERRCODE = 'no_data_found';
  END IF;
  ...
  SELECT COUNT(*) + 10 INTO l FROM portal.personsection WHERE por_id = por;
  -- offset and make space between indices
  UPDATE portal.personsection SET pse_order = l + 2 * pse_order WHERE por_id = por;
  ...
  i = 1;
  FOR mse IN
    SELECT pse_id FROM portal.personsection WHERE por_id = por ORDER BY pse_order
  LOOP
...
IS 'Move a section of a dossier view before a given position';
